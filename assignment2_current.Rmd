  ...
  for(k in 1:4) {
    ...
    for(z in 1:4) { c<- sd.err[z]
      for(j in 1:100){
           ...
           est.df[j,3]<-summary(m1)$coefficients["predictor","Estimate"]}}}}

est.df <- gather(est.df,value="est",keys="trial")
est.df$trial <- factor(est.df$trial)
...
for(i in 1:100){
  ...
  est[i] <- summary(m1)$coefficients["predictor","Pr(>|t|"]
  }
