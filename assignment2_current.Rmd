...
for(i in 1:4) {
  ...
  for(k in 1) {
    ...
    for(z in 1:4) { c<- sd.err[z]
      for(j in 1:100){
           m1<- lm(response ~ predictor,data=gen.mod(n[i],b0=0,b1[k],sd.err[z]))
           est.df[j,4]<-summary(m1)$coefficients["predictor","Pr(>|t|"]}}}}
...
I am not sure how to find what proportion of p-values are <=0.05
```
